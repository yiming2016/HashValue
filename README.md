...
👉 **直接下载最新发行版：<https://github.com/yiming2016/HashValue/releases>**

- 下载 `HashValue-vX.X.X-win64.zip` 后解压，双击 `HashValue.exe` 即可使用；
- 压缩包已内置 JtR 的 `run` 目录（`john.exe` 与各 2john 转换工具），**无需安装、无需自行下载任何依赖**；
- 绿色免安装，整个文件夹拷贝到其他电脑也能直接运行。
...
发行版压缩包已自带 JtR 的 `run` 目录，开箱即用；只有从源码编译运行时，才需要自行准备 2john 转换工具：

👉 **下载 John the Ripper：<https://github.com/openwall/john-packages/releases>**

- 从源码运行时，在窗口底部设置 JtR 目录（选择解压后含 `john.exe` 和 2john 脚本的 `run` 目录），之后会自动记住；
